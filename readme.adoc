...
. If this is the first time you are installing the AWS CLI:
** Configure the program by providing your credentials.
+
[source, bash]
----
aws configure
...
See https://docs.cloudera.com/cdp/latest/cli/topics/mc-cli-generating-an-api-access-key.html[CDP API Keys] for further details.
====
...
. Check you have a generated SSH keypair for your local profile.
** Read the https://www.ssh.com/academy/ssh/keygen[SSH Keygen How-To]

. Check you have a properly configured SSH Agent.
** Read the https://www.ssh.com/academy/ssh/keygen#adding-the-key-to-ssh-agent[SSH Agent How-To]
...
. Run the `quickstart.sh` entrypoint script.
+
[source, bash]
----
cd cloudera-deploy
...
. Confirm that you have the orange _cldr_ prompt.
+
====
...
. Modify your local `cloudera-deploy` user profile. This is present in your `$HOME` directory under `~/.config/cloudera-deploy/profiles/default`.
+
[source, bash]
----
